...
import { Schedule, Shifting } from '../../partimer/parttimer.model';
import { ParttimerserviceService } from '../../partimer/parttimerservice.service';
import { RouterLink } from '@angular/router';

@Component({
  selector: 'app-managerial-schedule',
  standalone: true,
  imports: [RouterLink],
  templateUrl: './managerial-schedule.component.html',
  styleUrl: './managerial-schedule.component.css'
})
export class ManagerialScheduleComponent implements OnInit {
  workShifts: Shifting[] = [];
    workSchedule: Schedule[] = [];
    days: {date:string; day:string; shifts:{partTimeId: number; partTimeNama:string;}[]}[] = [];
    ...
    private _initializeDays(startDate : Date){
      ...
      for (let i = 0; i < 6; i++) {
        ...
        const shifts = this.workShifts.map((_, shiftIndex) => {
          ...
          return {
            partTimeId: employee?.partTimeId ?? 0,
            partTimeNama: employee?.partTimeName ?? ""
          }
        });
